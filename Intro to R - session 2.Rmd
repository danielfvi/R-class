...
#Vectorizing
Vectorizing when we use vector operations to perform simulations, instead of populating a matrix. This makes the simulation much faster, which is important when you are running a sesitivity analysis over a complicated model. For this we use vector operations from session 1. Lets review some important vector operations and vectorize our simple population growth model over many growth rates:
...
plot(pop~growth, type="l", ylab="population size", xlab="growth rate", ylim=c(0, 11700000000))
...
Note that we cannot do the same graph as we did in session 1 because we only have a vector of the resulting pop size in year t. To do that we would have to create a matrix using the rbind function or construct a plot inside the loop. For example:
```{r}
#First we create a vector to store the parameter values (in this case the growth rate)
growth = seq(1.1, 2, by=0.1)
...
#Create a plot for our population sizes in every time setp. Note that we need to create a vector os the same size of pop repeating the corresponding time step
plot(rep(1, length(pop)), pop, type="l", ylab="population size", xlab="Years", ylim=c(0, 11700000000), xlim=c(1, t))
...
for(n in 2:t){
  ...
  #Alternatively, we can construct a matrix to store our population sizes from our vector out. This will add a line to out every time step.
  out = rbind(out, pop)
}
...
Functions are critical to modeling in R. By turning your model code (like the population model we did in session 1) into a function, it allows you to easily modify and test parameters, perform optimization and make your programming more efficient and clean. Like every other function in R, each function you create is an independent little program, performing a few related tasks and returning the results. This makes complex problems easier to program, and makes it easier to see the logical flow of a large program. The basic syntax for creating a function is as follows. 

Suppose you want a function mypop that produces our a population growth model. You first need to specify the name of the functions you wish to create (in this case mypop), and then use the function() command to create your function and inside the parentesis we will put the parameters of our model. In cases where you want a specific result (or results) from your function, you need the return() function in the end of your scrip to create mypop. The basic syntax is then:
...
#now we run a for loop through all our population growth rates
for(i in 1:length(growth)){
  res = mypop.last(g = growth[i], t=10, N0=10)
...
Write a function for the MPA model we constructed in exercise 2. For this model, our parameters will be $t$, $K$, $d$, $L$, $g$, $f$ and $MPA$. The parameter $MPA$ will be a value representing the percent in MPA of our model (from 0 to 100). Your should tranform this MPA value into a vector of fishing mortalities, where patches designated as MPA have a value of 0. Set $d=0.2$; $L=100$; $g=0.5$, $K=100$, $t=50$, and initital population of 50 individuals 

**a**) Calculate yield and population sizes for all MPA values (0, 1, 2..., 100). Test MPA efectiveness for fishing mortality values for patches open to fishing of $f=0.3$ and $f=0.6$. 
...
**b**) Calculate the optimal MPA size for different mobility values. Set $f=0.6$ for all patches open to fisheries and test values a range of d values (0, 0.1, 0.2...., 1) and MPA sizes (0, 1, 2..., 100). Optimal MPA size is defined as the percent in MPA that gives the maximum yield in year t. 
